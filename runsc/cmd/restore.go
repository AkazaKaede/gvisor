...
func (r *Restore) Execute(_ context.Context, f *flag.FlagSet, args ...any) subcommands.ExitStatus {
	...
	// If TestOnlyAutosaveStateFile is not empty, use it as the state file to
	// restore the sandbox. This flag should only be used in tests.
	if conf.TestOnlyAutosaveStateFile != "" {
